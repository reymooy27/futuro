...
export default function Tournament() {
  ...
  const { data, isLoading } =
    api.tournament.getTournamentById.useQuery(tournamentId);
      ...
      <>
        ...
        <Modal isOpen={isOpen} onClose={onClose}>
          ...
          <ModalContent>
            ...
            <ModalCloseButton />
            <ModalBody className="flex flex-wrap gap-3">
              {myTeam?.map((team) => (
                <div
                  onClick={() => joinTournament(team?.id)}
                  key={team?.id}
                  className="cursor-pointer rounded bg-slate-400 p-3"
                >
                  {team?.name}
                </div>
              ))}
        ...
        <div className="mt-3">
          ...
          <TableContainer>
            <Table variant="simple">
              ...
              <Tbody>
                {data?.teams.length < 1 && (
                  <div className="p-3">Tidak ada data tim</div>
                )}
