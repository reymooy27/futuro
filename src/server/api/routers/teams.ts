...
export const teamRouter = createTRPCRouter({
  ...
  getTeamById: publicProcedure.input(z.number()).query(({ ctx, input }) => {
    try {
  ...
  addTeamToTournament: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        const registered = await ctx.prisma.teamRegisterTournament.findFirst({
          where: {
            teamId: input.teamId,
        ...
        await ctx.prisma.teamRegisterTournament.create({
          data: {
            teamId: input.teamId,
            tournamentId: input.tournamentId,
            status: "PENDING",
        ...
        //   },
        // });
        return "Berhasil Mendaftarkan tim ke turnamen";
      } catch (error) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "There is something wrong",
